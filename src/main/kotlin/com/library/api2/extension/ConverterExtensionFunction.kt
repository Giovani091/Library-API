...
fun PostBooksRequest.toBooksModel(): BooksModel {
    return BooksModel(title = this.title, author = listOf(this.author), category = this.category)
}

fun PutBooksRequest.toBooksModel(id: Int): BooksModel{
    return BooksModel(id = id, title = this.title, author = listOf(this.author), category = this.category)
}